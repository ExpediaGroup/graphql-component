...
const filterResolvers = function (resolvers, excludes) {
  ...
  for (const [type, field] of excludes) {
    if (type === 'Query' || type === 'Mutation') {
      ...
      if (filteredResolvers[type] && Object.keys(filteredResolvers[type]).length === 0) {
        delete filteredResolvers[type];
      }
    }
  }
      ...
      for (const [field, resolver] of Object.entries(fieldResolvers)) {
        ...
        if (typeof resolver !== 'function') {
          ...
          result[type][field] = function (_, ...args) {
            // provides a "default resolver" type feel where we catch results
            // already computed in a child graphql execution context (such as 
            // when we delegate) to prevent the outer graphql execution
            // from running a non-root field resolver for the second time
            if (_.__typename || _[field]) {
