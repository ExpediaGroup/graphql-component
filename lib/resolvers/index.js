...
const filterResolvers = function (resolvers, excludes) {
  ...
  let filteredResolvers = cloneDeep(resolvers);
  ...
  for (const [type, fieldResolvers] of Object.entries(resolvers)) {
    ...
    if (['Query', 'Mutation', 'Subscription'].indexOf(type) === -1) {
      ...
      for (const [field, resolver] of Object.entries(fieldResolvers)) {
        ...
        // explicitly wrap __resolveType to short circuit if __typename is already resolved/present (on root)
        // this is to prevent double execution of the __resolveType resolver in delegation situations
        // when the delegatee resolves an interface type
        else if (field === '__resolveType') {
        ...
        // explicitly do not wrap __resolveReference - which is a special resolver for cross
        // service resolution of entities in federated schemas
        else if (field === '__resolveReference') {
          result[type][field] = resolver;
        }
        else {
          // otherwise wrap all other non-root field resolvers in an attempt to short circuit
          // and prevent double execution of non-root type field resolvers in delegation situations
          result[type][field] = function (root, args, context, info) {
            if (root[field]) {
              return root[field];
            }
