...
Test('filterResolvers()', (t) => {
  t.test('exclusions argument is undefined', (st) => {
    ...
    const filteredResolvers = filterResolvers(resolvers);
    st.deepEqual(filteredResolvers, resolvers, 'object content returned from filterResolvers is equal to input resolver object content');
    st.end();
  });
  ...
  t.test('exclusions for non root types are ignored', (st) => {
    const resolvers = {
      Query: {
        foo() { }
      },
    ...
    const filteredResolvers = filterResolvers(resolvers, [['SomeType']], [['AnotherType', '*']]);
    st.deepEqual(filteredResolvers, resolvers, 'object content returned from filterResolvers is equal to input resolver object content');
    st.end();
  })
  ...
  t.test(`exclude a entire type by specifying 'Type' exclusion`, (st) => {
    ...
    const filteredResolvers = filterResolvers(resolvers, [['Query']]);
    st.notOk(filteredResolvers.Query, 'entire type is excluded');
    st.ok(filteredResolvers.SomeType.bar, 'type not excluded remains');
    st.ok(resolvers.Query.foo, 'original resolver map maintains structure despite exclusions');
    st.ok(resolvers.SomeType.bar, 'original resolver map maintains structure despite exclusions');
    st.end();
  });
  ...
  t.test(`exclude an entire type by specifying 'Type.' exclusion`, (st) => {
    ...
    const filteredResolvers = filterResolvers(resolvers, [['Query', '']]);
    st.ok(filteredResolvers.SomeType.bar,'other non-excluded type remains');
    st.notOk(filteredResolvers.Query, 'entire specified type is excluded');
    st.ok(resolvers.Query.foo, 'original resolver map maintains structure despite exclusions');
    st.ok(resolvers.SomeType.bar, 'original resolver map maintains structure despite exclusions');
    st.end();
  });
  ...
  t.test(`exclude an individual field on a type`, (st) => {
    ...
    const filteredResolvers = filterResolvers(resolvers, [['Query', 'foo']]);
    st.notOk(filteredResolvers.Query.foo, 'specified field on specified type is removed');
    st.ok(filteredResolvers.Query.anotherQuery, 'non-excluded field on specified type remains');
    st.ok(filteredResolvers.SomeType.bar, 'non-exluded type remains');
    st.ok(resolvers.Query.foo, 'original resolver map maintains structure despite exclusions');
    st.ok(resolvers.Query.anotherQuery, 'original resolver map maintains structure despite exclusions');
    st.ok(resolvers.SomeType.bar, 'original resolver map maintains structure despite exclusions');
    st.end();
  });
  ...
  t.test('exclude all fields on a type via 1 by 1 exclusion', (st) => {
    ...
    const filteredResolvers = filterResolvers(resolvers, [['Query', 'foo'], ['Query', 'anotherQuery']]);
    st.notOk(filteredResolvers.Query, 'specified type is completely removed because all of its fields were removed');
    st.ok(filteredResolvers.SomeType.bar, 'non-exluded type remains');
    st.ok(resolvers.Query.foo, 'original resolver map maintains structure despite exclusions');
    st.ok(resolvers.Query.anotherQuery, 'original resolver map maintains structure despite exclusions');
    st.ok(resolvers.SomeType.bar, 'original resolver map maintains structure despite exclusions');
