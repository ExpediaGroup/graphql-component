...
Test('delegateToComponent - integration - non-root resolver in primitive only executed once', async (t) => {
  let fooDotBarRunCount = 0;
  const primitive = new GraphQLComponent({
    types: `
      type Query {
        foo: Foo
  ...
  const composite = new GraphQLComponent({
    ...
    resolvers: {
      ...
      CompositeType: {
        async foo(_root, _args, context, info) {
          const foo = await GraphQLComponent.delegateToComponent(primitive, {
            contextValue: context,
            info
          });
          return foo;
    ...
    ,
    imports: [ { component: primitive, exclude: ['Query.*']}]
  });
  ...
  t.equal(fooDotBarRunCount, 1, 'primitive Foo.bar non-root resolver only run 1 time');
  t.notOk(result.errors, 'no errors');
  t.deepEqual(result.data, { something: { foo: { id: '1', bar: { baz: 'foo id: 1'}}}}, 'result resolved as expected');
  t.end();
});
