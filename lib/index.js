...
class GraphQLComponent {
  constructor({ types = [], resolvers = {}, imports = [], fixtures = {}, directives = {}, context = {}, useFixtures = false }) {
    debug(`creating component schema`);

    this._types = (Array.isArray(types) ? types : [types]).map((type) => gql`${type}`);
    ...
    if (this._imports.length > 0) {
      ...
      if (this._types.length > 0) {
        ...
        //does not have own types, merge imports
        this._schema = GraphQLTools.mergeSchemas({
