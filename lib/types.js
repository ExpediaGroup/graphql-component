...
const renameDirectives = function (id, document) {
  for (const definition of document.definitions) {
    if (definition.kind === 'DirectiveDefinition') {
      definition.name.value += `_${id}`;
    }
  }
  return document;
};
...
const getImportedTypes = function (component, excludes) {
  const types = component._types.map((type) => renameDirectives(component._id, parse(type)));
