...
const createProxyResolver = function (component, root, field) {
  return async function (_, args, context, info) {
    ...
    const { data, errors } = await component.execute(createOperationForField(field, info), { context, variableValues });
    
    if (errors && errors.length > 0) {
      // Limitation: can only throw one error.
      // However, given this particular use case, you should only have one since you are only calling a sub-tree.
      throw errors[0];
    }
    return data[field];
  };
...
const getImportedResolvers = function (component) {
  return createProxyResolvers(component, mergeResolvers([component._resolvers, component._importedResolvers]));
};
